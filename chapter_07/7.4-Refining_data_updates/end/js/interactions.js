const populateFilters = () => {
  const filters = d3
    ...
    .attr(
      "class",
      (d) => `filter filter-${d.id} ${d.isActive ? "active" : ""}`,
    );
...
const handleClickOnFilter = (data) => {
  d3.selectAll(".filter").on("click", (e, datum) => {
    if (!datum.isActive) {
      // Update filters
      cetaceanFilters.forEach((h) => {
        h.isActive = h.id === datum.id ? true : false;
      });
      d3.selectAll(".filter").classed("active", (d) =>
        d.id === datum.id ? true : false,
      );
      ...
      // Reusable transition
      const t = d3.transition().duration(800).ease(d3.easeExpOut);
...
const hideTooltip = (e, d) => {
  d3.select(".tooltip").attr("y", -500).style("opacity", 0);
};
